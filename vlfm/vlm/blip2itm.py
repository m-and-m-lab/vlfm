...
class BLIP2ITM:
    ...
    def __init__(
        ...
        self.processor = Blip2Processor.from_pretrained(name)
        self.model = Blip2ForImageTextRetrieval.from_pretrained(name).to(device)
    ...
    def cosine(self, image: np.ndarray, txt: str) -> float:
        ...
        """
        pil_img = Image.fromarray(image)
        inputs = self.processor(images=pil_img, text=txt, return_tensors="pt").to(self.device)
        with torch.inference_mode():
            outputs = self.model(**inputs)
            itm_score = torch.softmax(outputs.itm_score, dim=1)
            cosine = itm_score[0, 1].item()
